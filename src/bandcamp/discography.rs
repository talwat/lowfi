...
impl DiscographyParser {
    /// Resolves mp3-128 stream URL from Bandcamp track page.
    pub async fn get_track_stream_url(client: &Client, track_url: &str) -> Result<Option<String>> {
        ...
        // Parse HTML to extract track data from script tag.
    ...
    // Fetch tracks for albums in parallel batches.
    async fn fetch_album_tracks(
        ...
        // Process albums and tracks in parallel batches.
    ...
    // Extract album/track links from HTML.
    ...
    // Extract metadata from embedded JSON data.
