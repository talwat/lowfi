...
pub struct List {
    ...
    /// Cache for color palettes extracted from cover art.
    #[allow(dead_code)]
    color_cache: std::collections::HashMap<String, Vec<[u8; 3]>>,
}
...
impl List {
    ...
    fn random_path(&self) -> (String, Option<String>, Option<String>) {
        ...
        if parts.len() >= 2 {
            ...
            if parts.len() >= 3 {
                let art_url = parts[2];
                // Extract cover art URL for UI coloring.
                let valid_art_url = if art_url.is_empty() || !art_url.starts_with("http") {
                    None
                } else {
    ...
    async fn expand_bandcamp(
                    ...
                    if let Some(tracks) = &item.tracks {
                        for track in tracks {
                            if !excluded_paths.iter().any(|ex| track.url.contains(ex)) {
                                ...
                                // Add cover art URL if present.
                                if let Some(art) = &item.image_url {
                                    entry.push('!');
                                    entry.push_str(art);
                                }
        ...
        for item in items {
            if let Some(tracks) = &item.tracks {
                let cached_tracks: Vec<cache::CachedTrackInfo> = tracks
                    .iter()
                    .filter(|track| !excluded_paths.iter().any(|ex| track.url.contains(ex)))
                    .map(|track| cache::CachedTrackInfo {
